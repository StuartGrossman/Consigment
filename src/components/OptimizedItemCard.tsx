...
const OptimizedItemCard: React.FC<OptimizedItemCardProps> = memo(({ 
  item, 
  isAdmin = false, 
  onClick,
  priority = false 
  ...
  const tags = useMemo(() => {
    const tagList: Array<{ label: string; color: string }> = [];
    if (item.category) tagList.push({ label: item.category, color: 'bg-blue-50 text-blue-700 border-blue-100' });
    if (item.brand) tagList.push({ label: item.brand, color: 'bg-purple-50 text-purple-700 border-purple-100' });
    if (item.size) tagList.push({ label: item.size, color: 'bg-green-50 text-green-700 border-green-100' });
    if (item.condition) tagList.push({ label: item.condition, color: 'bg-orange-50 text-orange-700 border-orange-100' });
    if (item.color) tagList.push({ label: item.color, color: 'bg-red-50 text-red-700 border-red-100' });
    return tagList;
  ...
  const hasMoreTags = useMemo(() => tags.length > 2, [tags.length]);
  
  const isOwnItem = useMemo(() => 
    !isAdmin && item.status === 'live' && user?.uid === item.sellerId, 
    [isAdmin, item.status, item.sellerId, user?.uid]
  );
  ...
  }, [item, user, isInCart, addToCart, removeFromCart]);

  const handleBookmarkAction = useCallback(async (e: React.MouseEvent) => {
    e.stopPropagation();
    await toggleBookmark(item.id, user);
  }, [item.id, user, toggleBookmark]);

  const handleTagToggle = useCallback((e: React.MouseEvent, show: boolean) => {
    stopPropagation(e);
    setShowAllTags(show);
  }, [stopPropagation]);
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/3] bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden">
        {images.length > 0 ? (
          <>
            ...
            {/* Image Navigation - Only show on hover and when appropriate */}
            {hasMultipleImages && isHovered && imageLoaded && (!user || isAdmin || item.status !== 'live' || user?.uid === item.sellerId) && (
              <>
                <button
            ...
            {/* Action Buttons Overlay - Only render when needed */}
            {canShowActions && (
            ...
            {/* Image Dots - Only show when appropriate */}
            {hasMultipleImages && isHovered && imageLoaded && (!user || isAdmin || item.status !== 'live' || user?.uid === item.sellerId) && (
      ...
      <div className="p-4 space-y-3 flex-1 flex flex-col">
        ...
        {/* Tags - Lazy loaded and optimized */}
        {tags.length > 0 && (
          <div className={`transition-all duration-300 ${isHovered ? 'opacity-100' : 'opacity-70'}`}>
            <div className="flex flex-wrap gap-1.5 items-center">
              ...
              {hasMoreTags && !showAllTags && (
                <button
                  onClick={(e) => handleTagToggle(e, true)}
              ...
              {showAllTags && hasMoreTags && (
                <button
                  onClick={(e) => handleTagToggle(e, false)}
                  className="bg-gray-50 text-gray-600 hover:bg-gray-100 px-2.5 py-1 rounded-full text-xs font-medium border border-gray-200 flex-shrink-0 transition-colors"
                >
                  −
                </button>
